...
object BytesTests extends Properties("Bytes") {
  ...
  property("EncodeDecodeChar") = forAll { (x:Char) => test(x) }    
  ...
  property("Option") = forAll { (x : Option[Short]) => Bytes.encode(x).decode == x.toVector }
  property("Tuple2") = forAll { (x : Short, y : Short) => Bytes.encode((x, y)).decode == Vector(x, y) }
  property("Tuple3") = forAll { (x : Short, y : Short, z : Long) => Bytes.encode((x, y, z)).decode == Vector(x, y, z) }
